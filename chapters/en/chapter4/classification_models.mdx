...
Finally, we can pass a sample to the classification pipeline to make a prediction (make sure `ffmpeg` [command line tool](https://ffmpeg.org/) and [`ffmpeg-python`](https://pypi.org/project/ffmpeg-python/) are installed on your machine):
```python
